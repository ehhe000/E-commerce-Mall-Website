<?php
	...
	if ($action == "add") { /* 保存购物车到数据库 */
		...
		$amount = $_GET["amount"];
		// 构建SQL语句
		$sql = "INSERT INTO carts VALUES(NULL, '$username', '$pid', '$name', '$price', '$amount')";
		// 执行SQL语句
		...
		$username = $_GET["username"];
		$amount = $_GET["amount"];
		$sql = "UPDATE carts SET proamount=$amount WHERE proid=$pid AND username='$username'";
		// 执行SQL语句
		$result = mysql_query($sql, $conn);
		if ($result)
			echo '{"status":1, "message":"success"}';
		...
		$pid = $_GET["pid"];
		$sql = "DELETE FROM carts WHERE proid=$pid";
		// 执行SQL语句
		$result = mysql_query($sql, $conn);
		if ($result)
			echo '{"status":1, "message":"success"}';
